...
void Physics::update(double t, float velocity, float acceleration, float degree)
{
	...
	m_friction = (0.05 * m_gravity * velocity) / m_pixelToMeter; // max friction = max acceleration

	m_velocity = velocity + (acceleration - m_friction) * t; // when max friction = max acceleration, velocity = 200
